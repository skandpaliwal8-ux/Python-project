import random                       #allows you to play against a bot
...
def possible_moves(position, player, r, c):           #function that displays all the possible moves for a player
    moves = []
    captures = []
    if player == 0: 
        directions = [(1, -1), (1, 1)]
...
def forced_move(position, player):                                     #function that displays all the captures for a given player
    forced = []
    pos_moves={}
...
def queen(position,move):                        #function that tracks when a piece reaches the end and becomes a king
...
def update_position(position, old_r, old_c, new_r, new_c, player):           #function that changes the position of the board when a move is made
    cap = False
    for piece in position[player]:
...
def win(position):                 #function that decides when a player has won a game
    if position[0]==[]:
        print("Player2 wins")
        return 0
    elif position[1]==[]:
...
def input_moves(prompt_text="Enter move: "):  #takes the input of the user
...
def bot_select(forced,pos_moves):            #function that allows you to play against the bot
    if forced:
        key = f"{forced[0][0]} {forced[0][1]}" #please note that the bot does not use any thinking and does random  moves
        moves = pos_moves[key]
